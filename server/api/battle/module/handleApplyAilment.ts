...
export const handleApplyAilment = (battleInfo: BattleInfo, playerOrEnemy: string, move: Move): BattleInfo => {
    // 必要データの確認
    ...
    switch (move.category) {
        case "damage":
            ...
            if (!damageResult) {
                console.error("handleApplyAilment: Failed to calculate damage");
                return null;
            }
...
const calcDamage = (attackPokemon: BattlePokemon, defencePokemon: BattlePokemon, move: Move): { defencePokemon: BattlePokemon , log: string } | null => {
    ...
    let attack = 0;
    let defence = 0;
    if(move.damage_class === "pysical")attack = attackPokemon.attack;
    if(move.damage_class === "special")attack = attackPokemon.special_attack;
    if(move.damage_class === "pysical")defence = defencePokemon.defence;
